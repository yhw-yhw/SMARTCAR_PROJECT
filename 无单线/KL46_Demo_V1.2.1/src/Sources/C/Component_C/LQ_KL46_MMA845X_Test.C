/********************************************************   
【平    台】龙丘KL46最小系统版
【编    写】龙丘
【Designed】by Chiu Sir
【E-mail  】chiusir@aliyun.com
【软件版本】V2.0
【最后更新】2013年12月24日
【相关信息参考下列地址】 
【网    站】http://www.lqist.cn
【淘宝店铺】http://longqiu.taobao.com
------------------------------------------------
【dev.env.】IAR6.50
【Target  】KL46
【Crystal 】50.000Mhz
------------------------------------
使用说明：串口发送实验
接线说明：RX  和地    系统版LED点亮
         短接PB22  和3.3v  系统版LED熄灭
使用北京龙邱智能科技cortex-M0开发工具箱
*********************************************************/#include "common.h"
#include "includes.h"
#include "LQ_KL46_MMA845X_Test.h"
#include "KL46_IIC.h"

unsigned int xy_mag;
unsigned int xz_mag;
unsigned int yz_mag;

signed  int xy_angle;
signed  int xz_angle;
signed  int yz_angle;

signed short accel_x, accel_y, accel_z;
signed short Aresultx, Aresulty, Aresultz;
char aux1;
int Atestx,Atesty,Atestz;

#define MMA8451_I2C_ADDRESS (0x1d<<1)
#define I2C0_B  I2C1_BASE_PTR
/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  accel_init
*  功能说明： 加速度计所需模块初始化
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
void accel_init(void)
{
    unsigned char tmp;
    hal_dev_mma8451_init();      //Initialize I2C modules
    tmp = hal_dev_mma8451_read_reg(0x2a);
    hal_dev_mma8451_write_reg(0x2a,tmp|0x01);
}

/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  hal_dev_mma8451_init
*  功能说明： 加速度计所需模块初始化
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
void hal_dev_mma8451_init(void)
{
  hal_i2c1_init(I2C0_B);
}


/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  static void pause(void)
*  功能说明：一个延时
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
static void pause(void)
{
    int n;
    for(n=0; n<40; n++)
       #ifndef CMSIS
           asm ("NOP"); // Toggle LED2
           #else
						__nop();
					 #endif
}

/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  static hal_dev_mma8451_read_reg pause(void)
*  功能说明： MMA8451的一个写操作
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
uint8 hal_dev_mma8451_read_reg(uint8 addr)
{
    uint8 result;

    i2c_start(I2C0_B);
    i2c_write_byte(I2C0_B, MMA8451_I2C_ADDRESS | I2C_WRITE);
    
    i2c_wait(I2C0_B);
    i2c_get_ack(I2C0_B);

    i2c_write_byte(I2C0_B, addr);
    i2c_wait(I2C0_B);
    i2c_get_ack(I2C0_B);

    i2c_repeated_start(I2C0_B);
    i2c_write_byte(I2C0_B, MMA8451_I2C_ADDRESS | I2C_READ);
    i2c_wait(I2C0_B);
    i2c_get_ack(I2C0_B);

    i2c_set_rx_mode(I2C0_B);

    i2c_give_nack(I2C0_B);
    result = i2c_read_byte(I2C0_B);
    i2c_wait(I2C0_B);

    i2c_stop(I2C0_B);
    result = i2c_read_byte(I2C0_B);
    pause();
    return result;
}


/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  static hal_dev_mma8451_write_reg pause(void)
*  功能说明： MMA8451的一个写操作
*  参数说明： addr  地址    data  数据
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
void hal_dev_mma8451_write_reg(uint8 addr, uint8 data)
{
    i2c_start(I2C0_B);

    i2c_write_byte(I2C0_B, MMA8451_I2C_ADDRESS|I2C_WRITE);
    i2c_wait(I2C0_B);
    i2c_get_ack(I2C0_B);

    i2c_write_byte(I2C0_B, addr);
    i2c_wait(I2C0_B);
    i2c_get_ack(I2C0_B);

    i2c_write_byte(I2C0_B, data);
    i2c_wait(I2C0_B);
    i2c_get_ack(I2C0_B);

    i2c_stop(I2C0_B);
    pause();
}



/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  accel_read
*  功能说明： MMA8451的一个读操作
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
void accel_read(void)
{
    if((hal_dev_mma8451_read_reg(0x00)&0xf) != 0)
    {
        accel_x   = hal_dev_mma8451_read_reg(0x01)<<8;
        accel_x  |= hal_dev_mma8451_read_reg(0x02);
        accel_x >>= 2;

        accel_y   = hal_dev_mma8451_read_reg(0x03)<<8;
        accel_y  |= hal_dev_mma8451_read_reg(0x04);
        accel_y >>= 2;

        accel_z   = hal_dev_mma8451_read_reg(0x05)<<8;
        accel_z  |= hal_dev_mma8451_read_reg(0x06);
        accel_z >>= 2;
          
        Aresultx   = hal_dev_mma8451_read_reg(0x01)<<8;
        Aresultx  |= hal_dev_mma8451_read_reg(0x02);
        Aresultx >>= 8;

        Aresulty   = hal_dev_mma8451_read_reg(0x03)<<8;
        Aresulty  |= hal_dev_mma8451_read_reg(0x04);
        Aresulty >>= 8;

        Aresultz   = hal_dev_mma8451_read_reg(0x05)<<8;
        Aresultz  |= hal_dev_mma8451_read_reg(0x06);
        Aresultz >>= 8;

    }
}

/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  accel_test
*  功能说明： MMA8451测试程序
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
void accel_test(void)
{
 accel_read();
 Atestz = Aresultz;
 
 printf("z =  %d \n\r",Atestz);
  
}

/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  iAcclReadX
*  功能说明： 角度的转换
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
int iAcclReadX(void)
{
    if((hal_dev_mma8451_read_reg(0x00)&0xf) != 0)
    {
        accel_x   = hal_dev_mma8451_read_reg(0x01)<<8;
        accel_x  |= hal_dev_mma8451_read_reg(0x02);
        accel_x >>= 2;
      
        Aresultx   = hal_dev_mma8451_read_reg(0x01)<<8;
        Aresultx  |= hal_dev_mma8451_read_reg(0x02);
        Aresultx >>= 8;
    }
    return Aresultx;
}

/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  iAcclReadY
*  功能说明： 角度的转换
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
int iAcclReadY(void) 
{
    if((hal_dev_mma8451_read_reg(0x00)&0xf) != 0)
    {
        accel_y   = hal_dev_mma8451_read_reg(0x03)<<8;
        accel_y  |= hal_dev_mma8451_read_reg(0x04);
        accel_y >>= 2;
        
        Aresulty   = hal_dev_mma8451_read_reg(0x03)<<8;
        Aresulty  |= hal_dev_mma8451_read_reg(0x04);
        Aresulty >>= 8;
    }
    return Aresulty;
}

/*************************************************************************
*                            LQ_KL46
*
*  函数名称：  iAcclReadZ
*  功能说明： 角度的转换
*  参数说明： 无
*  函数返回： 无
*  修改时间： 2013-11-19
*  作者    ：飞思卡尔官方库修改
*************************************************************************/
int iAcclReadZ(void) 
{
    if((hal_dev_mma8451_read_reg(0x00)&0xf) != 0)
    {
        accel_z   = hal_dev_mma8451_read_reg(0x05)<<8;
        accel_z  |= hal_dev_mma8451_read_reg(0x06);
        accel_z >>= 2;

        Aresultz   = hal_dev_mma8451_read_reg(0x05)<<8;
        Aresultz  |= hal_dev_mma8451_read_reg(0x06);
        Aresultz >>= 8;   
    }
    return Aresultz;
}
